...
class GPModel:
    def __init__(self):
        ...
        # Create dictionary to order size of x_test for each dimension
        dimension_size = {1: 1000,
                          2: 50,
                          3: 30,
                          4: 10}

        # Above 4D random sampling instead of linspace
        
        # Get size based on n
        selected_size = dimension_size.get(n, None)

        # Create the linspace tensors for each dimension
        grid_points = [torch.linspace(0, 1, selected_size) for _ in range(n)]
        
        # Generate an n-dimensional meshgrid
        grids = torch.meshgrid(*grid_points, indexing='ij')
        
        # Stack and flatten the meshgrid to create x_test
        self.x_test = torch.stack([grid.flatten() for grid in grids], dim=1).to(self.device)
