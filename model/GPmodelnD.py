...
class GPModelPipeline:
    def __init__(self, start_root_file_path=None, true_root_file_path=None, root_file_path=None, output_dir=None, initial_train_points=500, valid_points=1000, additional_points_per_iter=500, n_test=100000, n_dim=4, threshold=0.5):
        ...
        self.data_min = torch.tensor([-2000, -2000, 0, -2000], dtype=torch.float32).to(self.device)
        self.data_max = torch.tensor([2000, 2000, 60, 2000], dtype=torch.float32).to(self.device)
    ...
    def load_initial_data(self):
        ...
        #          5:  ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_M3"],
        #          6:  ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT"],
        #          7:  ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab"],
        #          8:  ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau"],
        #          9:  ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA"],
        #          10: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL"],
        #          11: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL"],
        #          12: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR"],
        #          13: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR", "IN_mtauR"],
        #          14: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR", "IN_mtauR", "IN_mqL1"],
        #          15: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR", "IN_mtauR", "IN_mqL1", "IN_mqL3"],
        #          16: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR", "IN_mtauR", "IN_mqL1", "IN_mqL3", "IN_muR"],
        #          17: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR", "IN_mtauR", "IN_mqL1", "IN_mqL3", "IN_muR", "IN_mtR"],
        #          18: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR", "IN_mtauR", "IN_mqL1", "IN_mqL3", "IN_muR", "IN_mtR", "IN_mdR"],
        #          19: ["IN_M_1", "IN_M_2", "IN_tanb", "IN_mu", "IN_AT", "IN_Ab", "IN_Atau", "IN_mA", "IN_meL", "IN_mtauL", "IN_meR", "IN_mtauR", "IN_mqL1", "IN_mqL3", "IN_muR", "IN_mtR", "IN_mdR", "IN_mbR"]
        #          }
    ...
    def load_additional_data(self):
        ...
        # Apply the mask to filter only valid Omega values
        Omega = al_df['MO_Omega'] # TODO: why is there no MO_Omega?
        mask = Omega > 0
        filtered_data = {param: al_df[f"{param}"][mask] for param in selected_columns}
        filtered_data['MO_Omega'] = Omega[mask]  # Always include Omega for filtering

        # Construct the limited DataFrame dynamically with only the selected columns
        limited_al_df = pd.DataFrame(filtered_data)

        additional_x_train = torch.stack([torch.tensor(limited_al_df[param].values[:self.additional_points_per_iter], dtype=torch.float32) for param in selected_columns], dim=1).to(self.device)
        additional_x_train = self._normalize(additional_x_train)

        additional_y_train = torch.log(torch.tensor(limited_al_df['MO_Omega'].values[:self.additional_points_per_iter], dtype=torch.float32).to(self.device) / 0.12)
    ...
    def goodness_of_fit(self, test=None, csv_path=None):
        ...
        # Convert to tensors
        input_data = torch.stack([torch.tensor(limited_df[param].values, dtype=torch.float32) for param in selected_columns], dim=1).to(self.device)
        input_data = self._normalize(input_data)
        true = torch.log(torch.tensor(limited_df['MO_Omega'].values, dtype=torch.float32).to(self.device) / 0.12)
...
if __name__ == "__main__":
    ...
    if args.iteration == 1:
        ...
        gp_pipeline = GPModelPipeline(
            ...
            start_root_file_path='/u/dvoss/al_pmssmwithgp/Run3ModelGen/source/Run3ModelGen/scans/scan_start4D/ntuple.0.0.root',
            true_root_file_path='/u/dvoss/al_pmssmwithgp/Run3ModelGen/source/Run3ModelGen/scans/scan_true4D/ntuple.0.0.root',
    ...
    if ActiveLearning:
        new_points, new_points_unnormalized = gp_pipeline.select_new_points(N=al_points)
    ...
    create_config(new_points=new_points_unnormalized, n_dim=n_dim, output_file ='new_config.yaml')
