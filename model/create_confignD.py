...
def create_config(new_points, output_file='output.yaml', n): # TODO: adjust create config in script with n
    # Create dictionary which orders the parameters based on n dimensions
    order = {1: ["M_1"],
             2: ["M_1","M_2"],
             3: ["M_1","M_2", "tanb"],
             4: ["M_1","M_2", "tanb", "mu"]}
    ...
    # Get the parameters to emptied based on n
    selected_params = order.get(n, [])

    # Update the parameters dictionary
    for key in parameters.keys():
        if key in selected_params:
            parameters[key] = [] # Set selected parameters to empty
        else:
            parameters[key] = parameters[key] # Leave other parameters unchanged

    # TODO: check if order of the parameters matter: for n=4 mu rises to the top
