...
class GPModelPipeline:
    ...
    def load_initial_data(self):
        ...
        limited_df = pd.DataFrame({'IN_M_1': M_1, 'IN_M_2': M_2, 'MO_Omega': Omega})
    ...
    def plotDifference(self, new_x=None, save_path=None, iteration=None):
        ...
        M_1 = self._normalize(M_1, self.data_min, self.data_max)[0]
        M_2 = self._normalize(M_2, self.data_min, self.data_max)[0]

        # Calculate the true values (log-scaled)
        true = torch.log(torch.tensor(Omega, dtype=torch.float32) / 0.12)

        # Evaluate model at M_1 and M_2 coordinates of true
        input_data = torch.stack([
            torch.tensor(M_1, dtype=torch.float32),
            torch.tensor(M_2, dtype=torch.float32)
        ...
        heatmap, xedges, yedges = np.histogram2d(M_1,
                                                M_2,
                                                bins=50, weights=diff.cpu().numpy())
        heatmap_counts, xedges, yedges = np.histogram2d(M_1, M_2, bins=50)
    ...
    def plotTrue(self, new_x=None, save_path=None, iteration=None):
        ...
        heatmap, xedges, yedges = np.histogram2d(M_1, M_2, bins=50, weights=true.cpu().numpy())
        heatmap_counts, xedges, yedges = np.histogram2d(M_1, M_2, bins=50)
        heatmap = heatmap/heatmap_counts
    ...
    def plotSlice1D(self, slice_dim=0, slice_value=0.75, tolerance=0.01, new_x=None, save_path=None, iteration=None):
        ...
        if self.true_root_file_path:
            ...
            print(f"M_1: {M_1}")
            print(f"M_2: {M_2}")
            print(f"M_1 Shape: {M_1.shape}")
            print(f"M_2 Shape: {M_2.shape}")

            # Normalize the true data
            if slice_dim == 0:
                indices_true = np.where((M_1 >= slice_value - tolerance) & (M_1 <= slice_value + tolerance))[0] # Creates tuple from which we take the first index
                x_true = M_2[indices_true] # Is marginalized along M_1, so M_2 is x_coordinate
            else:
                indices_true = np.where((M_2 >= slice_value - tolerance) & (M_2 <= slice_value + tolerance))[0]
                x_true = M_1[indices_true] # Is marginalized along M_2, so M_1 is x_coordinate
            y_true = torch.log(torch.tensor(Omega.values, dtype=torch.float32) / 0.12).cpu().numpy()
    ...
    def goodness_of_fit(self, test=None, csv_path=None):
        ...
        # Calculate the true values (log-scaled)
        true = torch.log(torch.tensor(Omega, dtype=torch.float32).to(self.device) / 0.12)
...
if __name__ == "__main__":
    ...
    # TODO: initialize amount training and valid points here
    if args.iteration == 1:
    ...
    if ActiveLearning:
        new_points, new_points_unnormalized = gp_pipeline.select_new_points(N=500)
